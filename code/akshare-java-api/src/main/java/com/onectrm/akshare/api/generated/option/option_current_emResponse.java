
/*generated by AKShare JAVA API Generator Sun Jun 12 15:42:02 CST 2022*/
package com.onectrm.akshare.api.generated.option;

import com.onectrm.akshare.api.JacksonDateDeserializer;
import com.onectrm.akshare.api.JacksonDateTimeDeserializer;
import com.onectrm.akshare.api.AkShareRequestAndResponse;
import com.fasterxml.jackson.annotation.*;
import com.fasterxml.jackson.databind.annotation.*;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.util.*;

/**
* 分类: option
* 接口: option_current_em
* 目标地址: http://quote.eastmoney.com/center/qqsc.html
* 描述: 东方财富网-行情中心-期权市场
* 限量: 单次返回全部合约的实时行情
*/
public class option_current_emResponse extends AkShareRequestAndResponse {
    public option_current_emResponse(){
        setInterfaceName("option_current_em");
    }

    public static option_current_emResponse of(


    Double p_ri4zeng1,

    Double p_jin1kai1,

    String p_ming2cheng1,

    Double p_zui4xin1jia4,

    Double p_cheng2jiao1liang4,

    Double p_zhang3die1e2,

    Double p_cheng2jiao1e2,

    Double p_zuo2jie2,

    Double p_zhang3die1fu2,

    Double p_chi2cang1liang4,

    String p_dai4ma3,

    Double p_sheng4yu2ri4,

    Double p_xing2quan2jia4,


    Map.Entry<String, String>... others){
    option_current_emResponse t = new option_current_emResponse();


    t.p_ri4zeng1 = p_ri4zeng1;

    t.p_jin1kai1 = p_jin1kai1;

    t.p_ming2cheng1 = p_ming2cheng1;

    t.p_zui4xin1jia4 = p_zui4xin1jia4;

    t.p_cheng2jiao1liang4 = p_cheng2jiao1liang4;

    t.p_zhang3die1e2 = p_zhang3die1e2;

    t.p_cheng2jiao1e2 = p_cheng2jiao1e2;

    t.p_zuo2jie2 = p_zuo2jie2;

    t.p_zhang3die1fu2 = p_zhang3die1fu2;

    t.p_chi2cang1liang4 = p_chi2cang1liang4;

    t.p_dai4ma3 = p_dai4ma3;

    t.p_sheng4yu2ri4 = p_sheng4yu2ri4;

    t.p_xing2quan2jia4 = p_xing2quan2jia4;


        for (Map.Entry<String, String> o : others) {
            t.other.put(o.getKey(), o.getValue());
        }

        return t;
    }



    /**
    * name: 日增
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("日增")
    
    
    private Double p_ri4zeng1;

    public Double getP_ri4zeng1() { return this.p_ri4zeng1;}
    public void setP_ri4zeng1(Double v) { this.p_ri4zeng1 = v;}

    /**
    * name: 今开
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("今开")
    
    
    private Double p_jin1kai1;

    public Double getP_jin1kai1() { return this.p_jin1kai1;}
    public void setP_jin1kai1(Double v) { this.p_jin1kai1 = v;}

    /**
    * name: 名称
    * type: object
    * desc: 
    * required: 
    */
    @JsonProperty("名称")
    
    
    private String p_ming2cheng1;

    public String getP_ming2cheng1() { return this.p_ming2cheng1;}
    public void setP_ming2cheng1(String v) { this.p_ming2cheng1 = v;}

    /**
    * name: 最新价
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("最新价")
    
    
    private Double p_zui4xin1jia4;

    public Double getP_zui4xin1jia4() { return this.p_zui4xin1jia4;}
    public void setP_zui4xin1jia4(Double v) { this.p_zui4xin1jia4 = v;}

    /**
    * name: 成交量
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("成交量")
    
    
    private Double p_cheng2jiao1liang4;

    public Double getP_cheng2jiao1liang4() { return this.p_cheng2jiao1liang4;}
    public void setP_cheng2jiao1liang4(Double v) { this.p_cheng2jiao1liang4 = v;}

    /**
    * name: 涨跌额
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("涨跌额")
    
    
    private Double p_zhang3die1e2;

    public Double getP_zhang3die1e2() { return this.p_zhang3die1e2;}
    public void setP_zhang3die1e2(Double v) { this.p_zhang3die1e2 = v;}

    /**
    * name: 成交额
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("成交额")
    
    
    private Double p_cheng2jiao1e2;

    public Double getP_cheng2jiao1e2() { return this.p_cheng2jiao1e2;}
    public void setP_cheng2jiao1e2(Double v) { this.p_cheng2jiao1e2 = v;}

    /**
    * name: 昨结
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("昨结")
    
    
    private Double p_zuo2jie2;

    public Double getP_zuo2jie2() { return this.p_zuo2jie2;}
    public void setP_zuo2jie2(Double v) { this.p_zuo2jie2 = v;}

    /**
    * name: 涨跌幅
    * type: float64
    * desc: 注意单位: %
    * required: 
    */
    @JsonProperty("涨跌幅")
    
    
    private Double p_zhang3die1fu2;

    public Double getP_zhang3die1fu2() { return this.p_zhang3die1fu2;}
    public void setP_zhang3die1fu2(Double v) { this.p_zhang3die1fu2 = v;}

    /**
    * name: 持仓量
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("持仓量")
    
    
    private Double p_chi2cang1liang4;

    public Double getP_chi2cang1liang4() { return this.p_chi2cang1liang4;}
    public void setP_chi2cang1liang4(Double v) { this.p_chi2cang1liang4 = v;}

    /**
    * name: 代码
    * type: object
    * desc: 
    * required: 
    */
    @JsonProperty("代码")
    
    
    private String p_dai4ma3;

    public String getP_dai4ma3() { return this.p_dai4ma3;}
    public void setP_dai4ma3(String v) { this.p_dai4ma3 = v;}

    /**
    * name: 剩余日
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("剩余日")
    
    
    private Double p_sheng4yu2ri4;

    public Double getP_sheng4yu2ri4() { return this.p_sheng4yu2ri4;}
    public void setP_sheng4yu2ri4(Double v) { this.p_sheng4yu2ri4 = v;}

    /**
    * name: 行权价
    * type: float64
    * desc: 
    * required: 
    */
    @JsonProperty("行权价")
    
    
    private Double p_xing2quan2jia4;

    public Double getP_xing2quan2jia4() { return this.p_xing2quan2jia4;}
    public void setP_xing2quan2jia4(Double v) { this.p_xing2quan2jia4 = v;}


    @JsonAnyGetter
    @JsonAnySetter
    private Map<String,String> other = new HashMap();
    public Map<String,String> getOther(){ return other; }
}
